...
- **Hero** — two photographs of the same five impasto strokes (ochre, dusty rose, orange, deep plum, burgundy) on artist's canvas, one per orientation: `paint-desktop.png` (2048×1152) for desktop from 1025px up, `paint-mobile.png` (1632×2048) for mobile and tablet below it. `tools/paint/hero_from_photo.py` owns the resulting hero files; `generate.py` deliberately no longer writes them, or a routine run would silently overwrite the artwork. **Only the portrait one is mirrored**, and they differ because the photographs do. Desktop needs empty ground on the left, where its headline, tagline and CTA sit, and the loaded ends on the right; the landscape photograph is already composed that way — a third of clear plaster, then the five strokes running out to the right with the brush still in shot — so it is used as shot, uncropped (frame and both candidates are all 16:9). The portrait photograph runs the other way, paint mass left and dry-brush right, so it is mirrored; that also keeps the artwork facing the same way at every width, so resizing past 1025px does not flip the strokes end for end. Mirroring is a property of each photograph, not a house rule. The portrait source needs no recomposition — it is shot in the orientation the mobile hero renders into, where the art it replaced was the landscape photo pasted low on a matching canvas ground to avoid losing three of the five strokes to a crop. Its dead bottom margin **is** trimmed, at row 1838 (the last row carrying pigment, measured off the file). Below 1025px the frame is always narrower than the artwork, so `object-fit: cover` fits by height and crops width — every row renders, and the photograph's bottom 210px of bare canvas showed as an empty band under the strokes at the foot of the hero. Trimming takes the source from 0.797:1 to 0.888:1 and puts the burgundy on the frame's bottom edge; nothing is discarded, since the cut is at the paint boundary rather than into it.
- **Known: the header phone number collides with the brush.** The landscape photograph puts a brush handle across the top-right corner, which is exactly where the transparent homepage header sits its phone link. Measured glyph-accurately, that link runs at **1.00–1.02:1** from 1025px to 2560px — effectively invisible where the digits cross the wood and the ferrule. The nav links (8.0–10.0:1) and the wordmark (11.2–11.6:1) are clear; only the phone is affected, and only at the top of the page, since the header turns opaque cream on scroll. No crop rescues it: `object-position` was swept across x and y, and above 1025px the frame is wider than the source so there is no horizontal slack to move, while the vertical crop that does exist at 1920px+ takes the nav down to 2.6–3.4:1 without helping the phone. The fix is on the header, not the image — a soft light scrim behind the transparent header, in the same language as the hero veil — and is deliberately not applied here.
