...
- **Hero stats band** — the trust row sits on a paint stroke of its own, in a warm tan from the same palette: same rounded ends and matte texture as every other stroke, just laid down solid so the icons and labels stay legible whatever is behind. Generated at two aspect ratios (the row is ~8:1 on desktop, ~3:1 on mobile) because stretching one asset across that range smears its texture. Band, veil and canvas all derive from one `--hero-canvas-rgb` triplet so they cannot drift apart. Set as token *values* at the top of `src/assets/css/main.css`; the token *names* are unchanged so the component CSS between them keeps resolving.
