...
- **Hero palette** — "Basic Canvas": one warm, dusty tonal family on a warm off-white canvas `#f8f3ea`, with nothing jumping out of it — mustard `#c9a34e`, blush pink `#c98e8e`, terracotta `#c06b4c`, cream/tan `#dfc3a6`, burgundy `#7e4450`. Deliberately low-chroma and matte; the strokes are rendered as pressed, blended swatches rather than glossy swipes (no specular rim, softened silhouette, canvas tooth showing through where the pigment is thin). Two brighter alternates (`studio`, `pop`) ship in the generator; switching is one command and colour-only.
- **Hero stats band** — the trust row sits on its own flat cream band rather than relying on the veil gradient, so its legibility does not depend on which stroke happens to be behind it. Band, veil and canvas all derive from one `--hero-canvas-rgb` triplet so they cannot drift apart. Set as token *values* at the top of `src/assets/css/main.css`; the token *names* are unchanged so the component CSS between them keeps resolving.
