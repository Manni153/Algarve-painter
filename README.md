...
- **Hero** — a photograph, `paint example.png`: five impasto strokes (ochre, dusty rose, orange, deep plum, burgundy) on artist's canvas. `tools/paint/hero_from_photo.py` owns the four hero files; `generate.py` deliberately no longer writes them, or a routine run would silently overwrite the artwork. The script does two things to the source: **mirrors it**, because the paint mass sits on the left of the photograph and the hero puts its headline there — mirrored, the dry-brush fade falls under the text and the loaded ends occupy the right; and **recomposes it for portrait**, laying the full width low on a matching canvas ground rather than cropping a 16:9 frame to 0.797 and losing three of the five strokes.
- **Hero resolution ceiling** — the source is 2048px wide, so the largest candidate is its native size, not an upscale. Every phone, tablet and standard-DPI desktop resolves to an asset at or above what it needs; a high-DPI desktop does not (1.9× at 1920 DPR 2). A light unsharp mask on the large candidates recovers some crispness, but the real fix is a higher-resolution original.
- **Generated marks** — the section swatches, CTA watermark, stats band and favicon are still generated (`tools/paint/generate.py`) in a palette chosen to sit with the photograph.
